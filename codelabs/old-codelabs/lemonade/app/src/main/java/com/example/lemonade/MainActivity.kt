...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        when (lemonadeState) {
            ...
            SQUEEZE -> handleSqueezeState()
            DRINK -> lemonadeState = RESTART
            RESTART -> lemonadeState = SELECT
        }

        Log.v(TAG, "State is now: ${lemonadeState}.")
        setViewElements()
    }
    ...
    private fun handleSelectState() {
        ...
        squeezeCount = 0
        Log.v(TAG, "Lemon size: ${lemonSize.toString()}.")
    }

    private fun handleSqueezeState() {
        lemonSize -= 1
        squeezeCount += 1
        checkIfLemonHasBeenSqueezed()
    }

    private fun checkIfLemonHasBeenSqueezed() {
        if (lemonSize == 0) {
            lemonadeState = DRINK
            lemonSize = -1
        }
